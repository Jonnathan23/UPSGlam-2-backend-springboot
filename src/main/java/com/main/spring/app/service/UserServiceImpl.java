        ...
        return userRepository.getUserById(userId)
                ...
                .flatMap(user -> {
                    ...
                    Mono<String> imageUrlMono = imageBytesMono
                            .flatMap(bytes -> supabaseStorageService.uploadImage(bytes, filePart.filename()));

                    // 4. Actualizar el photoUrl en Firestore
                    return imageUrlMono
                            .flatMap(photoUrl -> userRepository.updatePhotoUrl(userId, photoUrl))
                            .thenReturn("Foto de perfil actualizada correctamente")
                            .doOnSuccess(message -> System.out.println(
                                    "LOG: Foto de perfil actualizada para usuario: " + userId))
                            .onErrorResume(e -> {
                                System.err.println("ERROR: Fallo al actualizar foto de perfil. Causa: " + e.getMessage());
                                return Mono.error(new ResponseStatusException(
                                        HttpStatus.INTERNAL_SERVER_ERROR, "Error interno al actualizar la foto de perfil."));
                            });
                });
    }
        ...
        return userRepository.getUserById(userId)
                ...
                .flatMap(user -> {
                    ...
                    return userRepository.updateBio(userId, bio.trim())
                            ...
                            .doOnSuccess(message -> System.out.println(
                                    "LOG: Biografía actualizada para usuario: " + userId))
                            .onErrorResume(e -> {
                                System.err.println("ERROR: Fallo al actualizar biografía. Causa: " + e.getMessage());
                                return Mono.error(new ResponseStatusException(
                                        HttpStatus.INTERNAL_SERVER_ERROR, "Error interno al actualizar la biografía."));
                            });
                });
    }
}
